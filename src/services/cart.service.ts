...
export class CartService {
    ...
    //     return cart;
    //   }
    static async getCart(userId?: string, sessionId?: string): Promise<CartDocument> {
      ...
      static async mergeGuestCart(
        ...
        try {
          const [userCart, guestCart] = await Promise.all([
            this.getCart(userId),
            this.getCart(undefined, sessionId)
          ]);
          ...
          for (const item of guestCart.items) {
            ...
            const existingItem = userCart.items.find(
              userItem => userItem.product.toString() === item.product.toString()
            );
    ...
    static async updateQuantity(
        ...
        await cart.save();
        return cart.populate('items.product', 'name image price inStock');
    }
